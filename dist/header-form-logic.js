...
document.addEventListener("DOMContentLoaded", function() {
  ...
  initializePdfHandler("#partner_agreement_confirmation_btn", {
    ...
    webhookUrl: "https://prod-46.uksouth.logic.azure.com/workflows/0725daa15e4a42f996ff2c9b61d9b15f/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=9QTEWPyCdVCJrlTJe5VznGwnAVf_q9N3EthSalMQ8hA"
  });
  ...
  initializePdfHandler("#self_billing_confirmation_btn", {
    ...
    emailFieldName: "email",
    webhookUrl: "https://prod-09.uksouth.logic.azure.com:443/workflows/adc2994fde0c492981e334d10318e3cc/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=ODswH80OD7bbrkK12RH3snBpuArTY2Xkurs26K6jDwg"
  });
...
function setDropdownColor(selector, defaultColor = "#b1b1b1", selectedColor = "rgb(0, 0, 0)") {
...
async function generatePDFPartner(title, formData) {
  try {
    ...
    extractedContent.style.fontSize = "9px";
    ...
    extractedContent.style.lineHeight = "1.3";
    ...
    const pdf = await html2pdf().set(options).from(extractedContent).toPdf().get("pdf");
    const firstName = document.getElementById("Contacts-First-Name").value || "";
    const lastName = document.getElementById("Contact-Last-Name").value || "";
    const companyName = document.getElementById("Company-Details-Company-name").value || "";
    ...
    pdf.text("Signed by: Tom Wollin", 25, pageHeight - footerHeight + 10);
    pdf.text("On behalf of: plan.com", 25, pageHeight - footerHeight + 15);
    pdf.text(`Date: ${formattedDate}`, 25, pageHeight - footerHeight + 20);
    pdf.text(`Signed by: ${firstName} ${lastName}`, pageWidth / 2 + 25, pageHeight - footerHeight + 10);
...
function copyContactDetails(eventID, title, firstname, lastname, role, jobtitle, mobile, email, authorised, portal) {
...
function updateElementValue(sourceId, targetId) {
  ...
  document.getElementById(targetId).innerHTML = `<strong>${" " + sourceContent}</strong>`;
}
function addDynamicPlaceholder(selector, placeholderText) {
  document.querySelectorAll(selector).forEach((choicesContainer) => {
    ...
    if (!choicesInner || !selectedItems) return;
...
function initializeVATUploadCheckbox(checkboxId, uploadContainerId) {
  ...
  const errorMessage = vatUploadContainer.nextElementSibling;
...
async function generatePDFSelfBilling(title, formData) {
  ...
  doc.text(`Date: ${(/* @__PURE__ */ new Date()).toLocaleDateString()}`, 20, 280);
  doc.text(
...
function switchForm(checkboxSelector, buttonSelector, currentFormSelector, nextFormSelector) {
...
function toggleButtonWithClass(checkboxSelector, buttonSelector, activeClass = "next-button", inactiveClass = "inactive") {
  const checkbox = document.querySelector(checkboxSelector);
  const button = document.querySelector(buttonSelector);
  if (!checkbox || !button) {
...
async function sendToWebhook(payload) {
  const webhookUrl = "https://prod-56.uksouth.logic.azure.com/workflows/680656bfced4484ba11a1c5e51b400b9/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=wC1hD4ycXcwdJyhA30dbKlQ2NFMV7l0TTH8fPcKMN0U";
  const response = await fetch(webhookUrl, {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
...
//# sourceMappingURL=header-form-logic.js.map
